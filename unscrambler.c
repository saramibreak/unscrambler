/*
unscrambler 0.5.5: unscramble not standard IVs scrambled DVDs thru 
...
int test_seed(int j, unsigned long type) {
    ...
    if (type == 0 || type == 1) {
        ...
        for (k = 0; k < 12; k++) {
            memcpy(buf + 172 * k, b_in + 172 * k, 172);
        }
        edcPos = 2060;
    }
    else if (type == 3) {
...
int unscramble_frame(t_seed *seed, unsigned char *_bin, unsigned char *_bout, int blockSize, unsigned long type) {
    ...
    for(j=0; j< blockSize; j++) {
        ...
        if (type == 0 || type == 1) {
            ...
            bin = &_bin[0x8BC * j];
            for (k = 0; k < 12; k++) {
                memcpy(buf + 172 * k, bin + 172 * k, 172);
            }
...
int main(int argc, char *argv[]) {
    ...
    printf("GOD/WOD/DVD unscrambler 0.5.5 (xt5@ingenieria-inversa.cl)\n\n"
    ...
    if(argc<4) {
        printf(
            ...
            "      \t2: DVD 2236 bytes/sector, 3: DVD 2304 bytes/sector\n"
            "      \t4: DVD 2384 bytes/sector, 5: DVD 2816 bytes/sector\n"
            );
        return 0;
    }
    ...
    if (*endptr) {
        ...
        fprintf(stderr, "[%s] is invalid argument. Please input 0, 1, 2, 3, 4 or 5.\n", endptr);
        return -1;
    }
    ...
    blockSize = 16;
    if (type == 2) {
        readSize = (size_t)(0x8BC * blockSize);
        totalSectorSize = (int)(rawFileSize / 0x8BC);
    }
    else if (type == 3) {
        ...
        readSize = (size_t)(0xB00 * blockSize);
        totalSectorSize = (int)(rawFileSize / 0xB00);
    }
    else {
...
//            ret=5;
        ...
        if (s > totalSectorSize - blockSize && lastSectorsPerBlock > 0) {
            ...
            if (type == 2) {
                readSize = (size_t)(0x8BC * lastSectorsPerBlock);
            }
            else if (type == 3) {
                ...
                readSize = (size_t)(0xB00 * lastSectorsPerBlock);
            }
            else {
